...
const Essay = ({essay, parts}) => (
  <React.Fragment>
    ...
    {essay._requirements.length > 0 && parts.written_for && (
    ...
    {essay['Prompt'] && parts.prompt && (
      <InfoBox name="Prompt">
        <Paragraph>{essay['Prompt']}</Paragraph>
      </InfoBox>
    )}

    {essay['Brainstorming'] && parts.brainstorming && (
      <InfoBox name="Brainstorming">
        <Paragraph>{essay['Brainstorming']}</Paragraph>
      </InfoBox>
    )}

    {essay['Attachments'] && parts.attachments && (
  ...
  const report = (
    <html>
      ...
      <body style={pageStyles}>
        ...
        {essays
          .filter(e => (e.fields['Essay'] || '').trim().length > 0)
          .map(e => (
